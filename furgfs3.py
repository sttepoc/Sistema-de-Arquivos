...
class FURGfs3:
    ...
    def __init__(self, filename: str = None):
        ...
        self.script_dir = os.path.dirname(os.path.abspath(__file__))  # Pasta do script
    ...
    def _read_file_from_fs(self, filename: str) -> bytes:
        ...
        try:
            ...
            while current_block != 1 and bytes_left > 0:
                ...
                if next_block in [1, 0]: break  # Fim da cadeia
    ...
    def create_filesystem(self, filename: str, size_mb: int) -> bool:
        ...
        if not filename.endswith('.fs'): filename += '.fs'
    ...
    def _allocate_blocks(self, num_blocks: int) -> List[int]:
        ...
        if blocks: self.fat[blocks[-1]] = 1  # Marca fim da cadeia
        return blocks
    ...
    def _calculate_directory_size(self, directory_block: int) -> int:
        ...
        for entry in self._read_directory(directory_block):
            if entry['type'] == 0: total_size += entry['size']  # Arquivo
            elif entry['type'] == 1: total_size += self._calculate_directory_size(entry['start_block'])  # Subdiretório
        return total_size
    ...
    def _read_directory(self, directory_block: int = None, calculate_sizes: bool = False) -> List[Dict]:
        ...
        if directory_block is None: directory_block = self.current_directory
        ...
        for _ in range(self.BLOCK_SIZE // self.ENTRY_SIZE):
            ...
            try:
                ...
                name_bytes, size, start_block, timestamp, protected_val, entry_type = values[0], values[1], values[2], values[3], values[5], values[6]
                
                try: name = name_bytes.rstrip(b'\x00').decode('utf-8')
                except: continue
                ...
                if calculate_sizes and entry['type'] == 1:
                    try: entry['calculated_size'] = self._calculate_directory_size(entry['start_block'])
                    except: entry['calculated_size'] = 0
    ...
    def _write_directory_entry(self, entry: Dict, position: int = -1, directory_block: int = None):
        ...
        if position == -1: raise Exception("Diretório cheio")

        # Escreve entrada no diretório
        self.file_handle.seek(dir_position + position * self.ENTRY_SIZE)
        name_bytes = entry['name'].encode('utf-8')[:self.MAX_FILENAME].ljust(self.MAX_FILENAME, b'\x00')
    ...
    def change_directory(self, dirname: str) -> bool:
        ...
        for entry in entries:
            if entry['name'] == dirname and entry['type'] == 1:
                self.current_directory, self.directory_path = entry['start_block'], self.directory_path + [dirname]
                print(f"Diretório atual: {self.get_current_path()}")
                return True
    ...
    def _create_file_in_fs(self, filename: str, content: bytes) -> bool:
        ...
        for block in blocks:
            ...
            self.file_handle.write(chunk.ljust(self.BLOCK_SIZE, b'\x00'))  # Preenche com zeros
    ...
    def copy_to_fs(self, src_path: str, dst_name: str = None) -> bool:
        ...
        dst_name = dst_name.strip('"\'') if dst_name else os.path.basename(src_path)
        ...
        try:
            ...
            if success:
                ...
                if verification_content:
                    verification_hash = self._calculate_file_hash(verification_content)
    ...
    def copy_from_fs(self, src_name: str, dst_path: str) -> bool:
        ...
        try:
            ...
            if fs_hash == written_hash:
                ...
                print("❌ ERRO: Arquivo corrompido!")
                try: os.remove(dst_path)  # Remove arquivo corrompido
                except: pass
                return False
    ...
    def remove_file(self, filename: str) -> bool:
        ...
        for i, e in enumerate(entries_pos):
            if e['name'] == filename and e['type'] == 0:
                dir_position = self._get_directory_block_position(self.current_directory)
                self.file_handle.seek(dir_position + i * self.ENTRY_SIZE)
                self.file_handle.write(b'\x00' * self.ENTRY_SIZE)  # Zera entrada
                break
    ...
    def get_space_info(self) -> Tuple[int, int]:
        ...
        try:
            if not hasattr(self, 'fat') or not self.fat: return (0, 0)
    ...
    def get_current_path(self) -> str:
        ...
        if not hasattr(self, 'directory_path') or not self.directory_path: return "/"
...
def main():
    ...
    while True:
        ...
        current_path = fs.get_current_path() if hasattr(fs, 'filename') and fs.filename and hasattr(fs, 'get_current_path') else "/"
        ...
        try:
            ...
            if opcao == '1':
                ...
                nome, tamanho = input("Nome do sistema (sem .fs): "), int(input("Tamanho MB (1-10000): "))
                ...
                nome = input("Nome (.fs): ")
                if not nome.endswith('.fs'): nome += '.fs'
                full_path = os.path.join(fs.script_dir, nome)
                if os.path.exists(full_path): fs.filename = full_path
                elif os.path.exists(nome): fs.filename = nome
                else:
                    print("Arquivo não encontrado")
                    continue
                ...
                if opcao == '3':
                    origem, destino = input("Arquivo origem: "), input("Nome no FS (Enter=mesmo): ").strip()
                    fs.copy_to_fs(origem, destino if destino else None)
                elif opcao == '4':
                    origem, destino = input("Arquivo no FS: "), input("Destino: ")
                    ...
                    nome = input("Arquivo: ")
                    if input(f"Remover '{nome}'? (s/N): ").lower() == 's': fs.remove_file(nome)
                elif opcao == '7':
                    # Listagem formatada
                    arquivos = fs.list_files()
                    if arquivos:
                    ...
                    else: print("Nenhum item encontrado")
                elif opcao == '8':
                    free_f, used_f, total_f = fs.get_space_info_formatted()
                    print(f"Total: {total_f}, Usado: {used_f}, Livre: {free_f}")
                elif opcao == '9':
                    ...
                    if input(f"Remover '{nome}'? (s/N): ").lower() == 's': fs.remove_directory(nome)
                elif opcao == '14':
                    fs.verify_file_integrity(input("Arquivo para verificar integridade: "))
